...
public class TicketCheckController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		case "/TicketCheck":
			...
			String idcode = (String) session.getAttribute("mcode");
			...
			infodto3 = svc.selectCheckDrama(dcode);// 연극이름,감독이름,장르,가격,매수,상영하는날짜(시간),파일
		...
		case "/TicketCheckProcess":
			...
			String Checktmcode = request.getParameter("mcode");
			String Checktdcode = request.getParameter("dcode");
			int result = svc.InsertCheckTicket(Checktnum,Checktmcode, Checktdcode);
			if (result > 0) {
